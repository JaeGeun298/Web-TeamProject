...
| 팀원 | 담당 영역 | 상세 작업 |
|------|-----------|-----------|
| A (나) | 회원가입/로그인, 캘린더 | 회원가입 · 로그인 · 로그아웃 처리, 세션 인증, 캘린더 메인 화면, 여행 일정 캘린더 표시, 공통 레이아웃 |
| B | 지출 관리, 정산 | 지출 등록 · 수정 · 삭제, 카테고리 · 결제자 관리, 정산 계산 로직, 정산 결과 조회 |
| C | 여행 관리 | 여행 생성 · 조회, 멤버 초대 · 탈퇴, 공금 납입 기록 · 현황 조회, 여행 대시보드 |
